...
def train_model():
    ...
    for train_image in train_set:
        ...
        image = cv2.resize(copy.deepcopy(image), (desired_width, desired_height))
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        gray = filters.unsharp_mask(gray)

        mask = cv2.imread("healthy_fovmask/" + train_image + "_h_mask.tif")
        mask = cv2.resize(copy.deepcopy(mask), (desired_width, desired_height))
        mask = cv2.cvtColor(mask, cv2.COLOR_BGR2GRAY)
        mask = mask > 100
        gray[mask == 0] = 0

        model = cv2.imread("healthy_manualsegm/" + train_image + "_h.tif")
        model = cv2.resize(copy.deepcopy(model), (desired_width, desired_height))
...
def main():
    ...
    model = cv2.imread("healthy_manualsegm/01_h.tif")
    model = cv2.cvtColor(model, cv2.COLOR_BGR2GRAY)
    model = model > 10
    model[mask == 0] = 0
